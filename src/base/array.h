...
void _da_push(Arena *arena, T **arr, T val)
{
    ...
    {
        ArrayHeader *h0 = arr_getheader(*arr);
        U64 req = h0->size + 1;
        if (h0->capacity < req)
        {
...
#define da_push(arena, arr, val) (_da_push(arena, &arr, val), arr_getsize(arr) - 1)
...
#define va_push(arr, val) (_va_push(&arr, val), arr_getsize(arr) - 1)
